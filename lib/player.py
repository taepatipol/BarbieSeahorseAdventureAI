...
def pan_screen(g,s):
    ...
    g.view.top = min(g.view.top,border.top)
    g.view.right = max(g.view.right,border.right) 
    g.view.bottom = max(g.view.bottom,border.bottom)
    g.view.left = min(g.view.left,border.left)
